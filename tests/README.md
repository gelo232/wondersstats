...
Treize suites Playwright pilotent l'application réelle dans Chromium et vérifient
...
| Suite | Couverture |
|---|---|
| `smoke.js` | Migration v2 → v3 puis **v4 → v5** (les matchs d'un ancien tournoi se regroupent d'eux-mêmes, la nature est devinée, l'adversaire lu dans « vs X », l'opération est idempotente), navigation des onglets, stabilité de l'identité d'une joueuse (renommage + changement de numéro sans perte d'historique), détection des numéros en double, et la **reprise des naissances** : une base qui ne portait que l'année la garde au rechargement, une date reçue d'ailleurs redonne son année |
| `e2e.js` | Parcours complet : création de saison → effectif → vues sélectionneur (joueuse partagée entre deux vues) → **anonymat strict** (aucun nom dans le DOM ni dans le paquet exporté) → évaluation → soumission → compilation multi-évaluateurs → application des avis → composition de l'équipe → saisie de match → undo → persistance → suppression sans référence orpheline. Vérifie aussi qu'**aucun numéro n'est attribué automatiquement**, que la **réception compte ses trois niveaux** (l'ace subi inclus), que le **poste proposé** se choisit, se retire, remonte compilé chez l'entraîneuse **sans écraser le poste de l'effectif**, et qu'une entrée d'avant ces deux ajouts se normalise à vide. Vérifie qu'une **soumission reçue s'exporte en fichier et se réimporte à l'identique**, scores compilés compris — le seul filet contre une suppression sans retour. Éprouve enfin l'**ajout en lot avec naissance** (date complète, année seule, rien — le dossard restant le nombre de fin de ligne) et le **tri par date de naissance**, jour compris, dans les deux sens : l'année seule se range après les dates de son année, la fiche sans naissance ferme la marche |
| `modals.js` | Ouverture, rendu et fermeture de chacune des 21 modales, absence de fuite d'état entre modales, saisie manuelle du numéro, refus d'un doublon, **saisie d'une date de naissance** et refus d'un 31 février — dans la fiche comme dans l'ajout en lot, où une ligne fautive n'écrit aucune fiche. Vérifie aussi que la **catégorie a quitté la saison** sans quitter l'équipe, et l'onglet **🔢 Numérotation** : série devinée puis réglée, enchaînement automatique vers la suivante sans numéro, tuile prise portant le nom de son porteur, échange de deux dossards, retrait, et un effectif entier numéroté sans un seul doublon. Puis la **sélection multiple** des deux volets Joueuses : retirer plusieurs convocations d'un coup, l'avertissement qui nomme ce que le geste emporte, un vidage qui ne laisse aucune référence orpheline (effectif, compteurs, vues) sans toucher à la base, la convocation en lot dans l'ordre de tri choisi, l'archivage et sa réactivation, et une **suppression de fiches** qui efface jusqu'aux lignes de match et aux relevés d'évaluateur — sans laisser de joueuse sans fiche dans le cumul |
...
| `parcours.js` | **Le parcours opérationnel des quatre rôles**, sur quatre appareils : le propriétaire fonde et charte deux clubs, l'administratrice équipe le sien, l'entraîneuse convoque, joue un match et publie une vue, le sélectionneur évalue sur des numéros seuls puis soumet. Chaque étape est vérifiée deux fois — par l'état, et par une capture relue. Éprouve la boucle complète : ce que le sélectionneur saisit sur `#7` remonte nommément sur « Léa Tremblay » chez l'entraîneuse |
| `config.js` | **Le parcours de configuration, profil par profil** : ce que chacun doit régler, dans quel ordre, et ce que l'écran refuse tant qu'un préalable manque. Fondation et ses trois refus (nom, longueur, répétition), export scellé de la clé, branchement de GitHub avec un jeton d'abord invalide, assistant de relais avec son diagnostic, émission des jetons d'invitation. Puis l'arrivée des trois autres profils, qui n'ont rien à configurer : leur lien porte le relais. Enfin les **catégories** — liste par défaut sans U11 et avec U14, remaniement par l'administratrice, refus de retirer une catégorie portée par une équipe, et survie d'une catégorie héritée dans le menu — et la **cohérence des champs pendant la saisie** |
| `invitation.js` | **Le parcours d'une invitation, de bout en bout, à vingt-huit athlètes** : l'entraîneure ajoute une personne, l'affecte comme sélectionneuse, émet son jeton depuis la modale et partage le lien ; la vue est publiée **sans groupes, qui ne sont pas du ressort de l'entraîneure** ; la sélectionneuse rejoint sur son propre appareil, **compose elle-même quatre groupes de sept** puis déplace un numéro de l'un à l'autre à la main, évalue vague par vague, replie la grille, masque les faites, soumet ; l'entraîneure relève et retrouve ses vingt-huit athlètes nommément. Vérifie les défauts d'origine : une invitation de sélectionneur qui **ne doit jamais devenir une administration** quand le salon a expiré, une vue reçue par relais qui **doit porter son équipe** sans quoi le relais refuse la soumission, les **groupes qui suivent la vue** — préparés par l'entraîneure, reçus par l'invité, recomposés par lui, intacts après un rechargement et après une republication — l'**invitation lancée depuis une vue** — un second évaluateur invité en un geste, qui ne reçoit que sa vue quand l'autre ne reçoit que la sienne — et les **chemins de travers de l'invitation** — l'application qui n'écrit plus de base vide avant d'avoir rien chargé, le lien touché sur une application déjà ouverte, une base vide qui ne prime pas sur l'invitation, un lien coupé qui le dit |
